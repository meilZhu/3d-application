    ...
    <el-container class="three-app" v-loading="isLoading">
      ...
      <el-aside class="three-app-aside" :width="asideWidth + 'px'" v-if="leftMenuWidth">
        ...
        <my-model-tree
          ...
          :isHiddenLoadingModel="isHiddenLoadingModel"
          v-model="preKey"
      ...
      <el-main class="three-app-main" id="app">
        <div class="g-pos-relative" id="engineWrapper">
          <div
            ...
            :style="{
              height: engineHeight + 'px',
              width: engineWidth + 'px',
              border: 'none',
            }"
          >
            <div
              id="JT_WEB"
              class="siemens-app-container"
        ...
        <div
          class="menu-move"
          :class="[
            menuPosition === 'topLeft' ? 'menu-top-left' : '',
        ...
        >
          <my-model-menu :modelData="modelData" :selectedKeys="selectedKeys">
            ...
            <el-tooltip class="item" effect="light" content="展开操作菜单" placement="bottom">
              <i
                v-if="!isHiddenMenuCollpase"
                class="fa drop"
                :class="[
                  !isMenuCollapse ? 'drop-down-hide' : '',
                  menuPosition === 'topLeft' || menuPosition === 'topRight'
                    ? 'drop-down'
                    : 'drop-bottom-down',
                  menuPosition === 'topLeft' || menuPosition === 'topRight'
                    ? 'fa-angle-down'
...
export default {
  ...
  methods: {
    ...
    clearEngine() {
      window.jtWeb = null
      const jtWeb = document.querySelector('#JT_WEB')
      if (jtWeb) {
        jtWeb.parentNode.removeChild(jtWeb)
      }
      ...
      const jtEle = `
        ...
        ></div>
      `
      const content = document.querySelector('#jt-web-contain')
      content.innerHTML = jtEle
    },

    // 预缓存的加载事件
    preLoadToCache(data) {
      if (window.viewerManager) {
        window.jtWeb.preLoadToCache(data)
      } else {
        this.createJtWeb()
        window.jtWeb.preLoadToCache(data)
      }
      ...
      return {
        ...
        actions: {
          ...
          onLoad() {
            ...
            // 测点双击事件
            window.viewerManager.viewer.addEventListener('dblSelectPointEvent', pointObj => {
              _self.$emit('dblSelectPointEvent', pointObj)
            })
            // 鼠标移入测点事件
            window.viewerManager.viewer.addEventListener('pointEnterEvent', pointObj => {
              _self.$emit('pointEnetrEvent', pointObj)
            })
            // 鼠标移出测点事件
            window.viewerManager.viewer.addEventListener('pointLeaveEvent', pointObj => {
              _self.$emit('pointLeaveEvent', pointObj)
            })
          },
          onLoadComplete() {
            // 做延迟，保证模型已经加载好
          ...
          onPreLoadToCacheComplate() {
            // 预缓存处理完成
            _self.$emit('onPreLoadToCacheComplate')
          },
        },
    ...
    change3dSize() {
      let dom
      if (this.$el) {
        dom = this.$el.querySelector('#Control_0')
      }
      if (dom) {
        dom.style.width = `${this.engineWidth}px`
      }
    },
